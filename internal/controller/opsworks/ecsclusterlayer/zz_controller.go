...
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	...
	eventHandler := handler.NewEventHandler(handler.WithLogger(o.Logger.WithValues("gvk", v1beta1.EcsClusterLayer_GroupVersionKind)))
	ac := tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1beta1.EcsClusterLayer_GroupVersionKind), tjcontroller.WithEventHandler(eventHandler), tjcontroller.WithStatusUpdates(false))
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginSDKAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["aws_opsworks_ecs_cluster_layer"],
				...
				tjcontroller.WithTerraformPluginSDKAsyncMetricRecorder(metrics.NewMetricRecorder(v1beta1.EcsClusterLayer_GroupVersionKind, mgr, o.PollInterval)),
	...
	// register webhooks for the kind v1beta1.EcsClusterLayer
	// if they're enabled.
	if o.StartWebhooks {
		if err := ctrl.NewWebhookManagedBy(mgr).
			For(&v1beta1.EcsClusterLayer{}).
			Complete(); err != nil {
			return errors.Wrap(err, "cannot register webhook for the kind v1beta1.EcsClusterLayer")
		}
	}
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1beta1.EcsClusterLayer{}, eventHandler).

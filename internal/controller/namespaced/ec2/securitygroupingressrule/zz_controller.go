...
package securitygroupingressrule
...
// SetupWebhookWithManager registers the conversion webhook for SecurityGroupIngressRule.
func SetupWebhookWithManager(mgr ctrl.Manager) error {
	if err := ctrl.NewWebhookManagedBy(mgr, &v1beta1.SecurityGroupIngressRule{}).
		Complete(); err != nil {
		return errors.Wrap(err, "cannot register webhook for the kind v1beta1.SecurityGroupIngressRule")
	}
	return nil
}

// SetupGated adds a controller that reconciles SecurityGroupIngressRule managed resources.
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1beta1.SecurityGroupIngressRule_GroupVersionKind.String())
		}
	}, v1beta1.SecurityGroupIngressRule_GroupVersionKind)
	return nil
}

// Setup adds a controller that reconciles SecurityGroupIngressRule managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1beta1.SecurityGroupIngressRule_GroupVersionKind.String())
	var initializers managed.InitializerChain
	for _, i := range o.Provider.Resources["aws_vpc_security_group_ingress_rule"].InitializerFns {
	...
	if o.MetricOptions != nil && o.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), o.Logger, o.MetricOptions.MRStateMetrics, &v1beta1.SecurityGroupIngressRuleList{}, o.MetricOptions.PollStateMetricInterval,
		)
		if err := mgr.Add(stateMetricsRecorder); err != nil {
			return errors.Wrap(err, "cannot register MR state metrics recorder for kind v1beta1.SecurityGroupIngressRuleList")
		}
	}
	...
	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1beta1.SecurityGroupIngressRule_GroupVersionKind), opts...)

	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o.ForControllerRuntime()).
		WithEventFilter(xpresource.DesiredStateChanged()).
		Watches(&v1beta1.SecurityGroupIngressRule{}, eventHandler).
